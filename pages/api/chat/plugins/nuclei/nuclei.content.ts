...
import { isInteger } from '@/pages/api/chat/plugins/tools';
...
const displayHelpGuide = () => {
  return `
  [Nuclei](https://github.com/projectdiscovery/nuclei) is a fast exploitable vulnerability scanner designed to probe modern applications, infrastructure, cloud platforms, and networks, aiding in the identification and mitigation of vulnerabilities. 
    ...
    OPTIMIZATIONS:
       -timeout int               time to wait in seconds before timeout (default 15)
...
const parseCommandLine = (input: string) => {
  ...
  const isValidDomain = (domain: string) =>
    ...
    isWithinLength(domain, MAX_DOMAIN_LENGTH);
  ...
  const isValidUrlOrDomain = (value: string) => {
    ...
    try {
      ...
      return (
        /^[a-zA-Z0-9.-]+$/.test(value) && value.length <= MAX_DOMAIN_LENGTH
      );
    }
  };
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '-include-templates':
        if (isValidCommaSeparatedList(nextArg, isValidString)) {
          params.includeTemplates = nextArg
            .split(',')
            .map((item) => item.trim());
          i++;
        } else {
      ...
      case '-exclude-templates':
        if (isValidCommaSeparatedList(nextArg, isValidString)) {
          params.excludeTemplates = nextArg
            .split(',')
            .map((item) => item.trim());
          i++;
        } else {
      ...
      case '-exclude-matchers':
        if (isValidCommaSeparatedList(nextArg, isValidString)) {
          params.excludeMatchers = nextArg
            .split(',')
            .map((item) => item.trim());
          i++;
        } else {
      ...
      case '-exclude-severity':
        if (isValidCommaSeparatedList(nextArg, isValidString)) {
          params.excludeSeverity = nextArg
            .split(',')
            .map((item) => item.trim());
          i++;
        } else {
      ...
      case '-template-condition':
        if (isValidCommaSeparatedList(nextArg, isValidString)) {
          params.templateCondition = nextArg
            .split(',')
            .map((item) => item.trim());
          i++;
        } else {
...
export async function handleNucleiRequest(
  ...
  // OPTIMIZATIONS
  if (params.timeout !== 15) requestBody.timeout = params.timeout;
...
function formatResponseString(urls: any[], params: NucleiParams) {
  ...
  return (
    '## [Nuclei](https://github.com/projectdiscovery/nuclei) Scan Results\n' +
    '**Target**: "' +
    params.target +
    '"\n\n' +
    '**Scan Date and Time**:' +
